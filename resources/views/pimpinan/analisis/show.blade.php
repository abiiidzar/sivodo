...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div class="flex items-center gap-4">
            ...
            <div class="ml-auto flex items-center gap-2">
                <span class="text-xs px-2 py-1 bg-gray-100 text-gray-500 rounded-full">Read-Only</span>
                <a href="{{ route('pimpinan.analisis.index') }}"
    ...
    @if(count($kelemahan) > 0)
        <div class="bg-red-50 border border-red-200 rounded-xl p-6">
            ...
            <p class="text-xs text-red-600 mt-1">Aspek dengan rata-rata di bawah 3.5 memerlukan perhatian khusus</p>
                    ...
                    <tr class="border-b border-gray-50 hover:bg-gray-50 transition">
                        ...
                        <td class="py-2 px-3 max-w-[150px]">
                            @if($item->kritik || $item->saran)
                                <button onclick="showFeedback('{{ addslashes($item->kritik ?? '-') }}', '{{ addslashes($item->saran ?? '-') }}')"
    ...
    <div class="flex flex-wrap items-center gap-3">
        <a href="{{ route('pimpinan.analisis.export-pdf', $dosen->id) }}"
        ...
        <a href="{{ route('pimpinan.analisis.export-excel', $dosen->id) }}"
...
<div id="feedbackModal" class="fixed inset-0 z-50 hidden">
    <div class="fixed inset-0 bg-black/50" onclick="closeFeedback()"></div>
    <div class="fixed inset-0 flex items-center justify-center p-4">
        <div class="bg-white rounded-xl shadow-xl max-w-lg w-full p-6 relative">
            <div class="flex justify-between items-center mb-4">
                <h4 class="font-semibold text-navy">Kritik & Saran</h4>
                <button onclick="closeFeedback()" class="text-gray-400 hover:text-gray-600">
            ...
            <div class="space-y-4">
                <div>
                    <p class="text-sm font-medium text-navy">Kritik</p>
                    <p id="kritikText" class="text-sm text-gray-600 p-3 bg-gray-50 rounded-lg"></p>
                </div>
                <div>
                    <p class="text-sm font-medium text-navy">Saran</p>
                    <p id="saranText" class="text-sm text-gray-600 p-3 bg-gray-50 rounded-lg"></p>
                </div>
            </div>
            <div class="mt-4 text-right">
                <button onclick="closeFeedback()" class="px-4 py-2 bg-navy text-white rounded-lg hover:bg-navy/90 transition">
                    Tutup
                </button>
            </div>
        </div>
...
@push('scripts')
<script>
    function showFeedback(kritik, saran) {
        document.getElementById('kritikText').textContent = kritik || '- Tidak ada kritik -';
        document.getElementById('saranText').textContent = saran || '- Tidak ada saran -';
