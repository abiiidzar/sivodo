...
<div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div class="flex flex-wrap items-center gap-3 w-full sm:w-auto">
                ...
                <div class="flex items-center w-full sm:w-56 h-11 border border-gray-300 rounded-lg overflow-hidden">
                    ...
                    <div class="flex-1">
                        <input type="text" id="searchRiwayat" placeholder="Cari Nama Dosen..."
            ...
            <span class="text-sm text-gray-500">Total: {{ $votings->total() }} voting</span>
        </div>
    </div>
    ...
    @if($votings->count() == 0)
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
            ...
            <a href="{{ route('mahasiswa.voting') }}" class="text-gold hover:underline text-sm mt-2 inline-block">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 hover:shadow-md transition">
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <span class="px-3 py-1 rounded-full text-xs font-medium text-white {{ $this->getKategori($voting->rata_rata)['class'] }}">
                            {{ $this->getKategori($voting->rata_rata)['label'] }}
                        </span>
                        <a href="{{ route('mahasiswa.riwayat.show', $voting->id) }}"
                           class="text-gold hover:text-gold/80 transition">
...
<script>
    document.getElementById('btnSearch').addEventListener('click', function() {
        const search = document.getElementById('searchRiwayat').value;
        let url = '{{ route("mahasiswa.riwayat") }}?';
    ...
    document.getElementById('searchRiwayat').addEventListener('keypress', function(e) {
        if (e.key === 'Enter') {
