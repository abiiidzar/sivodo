        ...
        <form action="{{ route('admin.dosen.update', $dosen->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <select id="status_dosen" name="status_dosen"
                    ...
                    <option value="PNS" {{ old('status_dosen', $dosen->status_dosen) == 'PNS' ? 'selected' : '' }}>PNS</option>
                    <option value="Yayasan" {{ old('status_dosen', $dosen->status_dosen) == 'Yayasan' ? 'selected' : '' }}>Yayasan</option>
                    <option value="Luar Biasa" {{ old('status_dosen', $dosen->status_dosen) == 'Luar Biasa' ? 'selected' : '' }}>Luar Biasa</option>
                </select>
                @error('status_dosen')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
